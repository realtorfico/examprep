...
// The "Purchase (1)" conversion action in the Google Ads account whose tag the page loads
// (AW-18460635935, see gtag-init.js), which replaced the original account (AW-1046929025) on
// 2026-09-18. Only the send_to is taken from Google's event snippet -- its click-handler wrapper and
// fixed 36.99 value don't apply here, since this fires on a completed purchase with the real amount.
var GOOGLE_ADS_PURCHASE_SEND_TO = 'AW-18460635935/jF5zCJS-1_wcEJ_m2-JE';
...
//
// Lives in wwwroot/js/content/<kind-slug>.js since 2026-09-17, one file per exam kind, loaded on
// demand -- those files are the source of truth and are edited directly (a track's content work is
// scoped to one kind anyway). As a single literal in this file the catalog was 187KB minified and
// 24.5KB brotli: 42% of the bundle's parse cost and 30% of its wire size, on every page, when a CDL
// landing page needs 50 of its 290 entries. See test/track-content-split.test.js and
// scripts/build-track-content.js (the one-time extraction).
var HUB_EXAMS_CONTENT = [];
// Bump when any wwwroot/js/content/*.js file changes -- /js/* is served immutable (see _headers),
// and _worker.js preloads the same URL, so keep its copy of this number in step.
var TRACK_CONTENT_VERSION = 1;
// The content files that actually exist, which is NOT the same as the list of kind slugs: MLO has
// no file of its own because its single entry carries the '#' route placeholder and lives in
// unrouted.js. Asking for a file that doesn't exist would leave the first render waiting on a 404
// (it did -- the homepage hung on 'mlo'), so every load goes through this list.
// test/track-content-split.test.js asserts it matches the directory exactly.
var TRACK_CONTENT_SLUGS = ['act', 'boating', 'cdl', 'clt', 'dat', 'driver', 'motorcycle', 'notary', 'oat', 'real-estate-broker', 'real-estate-salesperson', 'unrouted'];
var trackContentLoaded = {};
var trackContentRequests = {};
var trackContentResolvers = {};
// Called by each content file as it arrives. Rebuilds HUB_EXAMS straight away when the registry is
// already in hand, so a page that renders before a later kind lands still picks it up afterwards.
//
// Registration -- not the <script> tag's load event -- is what settles a pending
// loadTrackContentFile(): "the content is available" is the thing callers actually wait on, and it
// holds however the file arrived (injected here, shipped in the HTML, or eval'd by a test harness).
function registerTrackContent(slug, entries) {
  if (trackContentLoaded[slug]) return;
  trackContentLoaded[slug] = true;
  HUB_EXAMS_CONTENT = HUB_EXAMS_CONTENT.concat(entries);
  if (lastRegistryTracks) HUB_EXAMS = buildHubExams(lastRegistryTracks);
  if (trackContentResolvers[slug]) { trackContentResolvers[slug](); delete trackContentResolvers[slug]; }
}
...
function loadTrackContentFile(slug) {
  if (trackContentLoaded[slug]) return Promise.resolve();
  if (trackContentRequests[slug]) return trackContentRequests[slug];
  trackContentRequests[slug] = new Promise(function (resolve) {
    trackContentResolvers[slug] = resolve;
    var s = document.createElement('script');
    s.src = '/js/content/' + slug + '.js?v=' + TRACK_CONTENT_VERSION;
    // onload is a backstop: the file calls registerTrackContent() as it runs, which resolves this
    // already. onerror resolves too -- a missing content file must not hang the first render, and
    // buildHubExams() still derives a usable route and name for every track it's missing.
    s.onload = function () { resolve(); };
    s.onerror = function () { resolve(); };
    document.head.appendChild(s);
  });
  return trackContentRequests[slug];
}

// What this pathname needs registered before it can render.
//
// A category or track page ('/cdl', '/cdl/ca') needs exactly its own kind -- that's the whole point
// of the split, and it's where the ad traffic lands. Anywhere else (the homepage's category grid,
// /blog, a hash route) reads titles across kinds, directly or through the sitewide footer's
// cross-category links, so those gate on the full catalog exactly as they did before the split --
// rendering them from buildHubExams()'s derived fallbacks and correcting a beat later would be the
// same late-content shift the first-paint work just removed.
//
// 'unrouted' always comes along: it holds the MLO scaffold entry, whose route is the '#'
// placeholder, so no pathname would ever ask for it by name. It is under 2KB.
function pageTrackContentSlugs() {
  var first = (location.pathname.split('/')[1] || '').toLowerCase();
  if (first && kindFromSlug(first) && TRACK_CONTENT_SLUGS.indexOf(first) !== -1) return ['unrouted', first];
  return TRACK_CONTENT_SLUGS.slice();
}
function loadPageTrackContent() {
  return Promise.all(pageTrackContentSlugs().map(loadTrackContentFile));
}
// Everything else, once the page is up: an in-app navigation to another category (or the footer's
// cross-category links) needs those titles, and by then the fetch is free.
function loadRemainingTrackContent() {
  return Promise.all(TRACK_CONTENT_SLUGS.map(loadTrackContentFile));
}
...
var HUB_EXAMS = [];
// Kept so registerTrackContent() can rebuild HUB_EXAMS when a kind's content arrives after the
// registry has already been merged -- which is the normal case now that content loads per page.
var lastRegistryTracks = null;
...
function buildHubExams(registryTracks) {
  ...
  return registryTracks.map(function (r) {
    var content = contentByType[r.examType];
    if (content) return Object.assign({}, r, content);
    // This kind's content file hasn't loaded (only the current page's does before first render).
    // The registry has no route or long title of its own, so derive both rather than render
    // "undefined" or a dead link in the window before loadRemainingTrackContent() lands:
    // '<kind slug>/<state>' is every routed track's real URL, asserted for all 288 of them in
    // test/track-content-split.test.js.
    var slug = kindSlug(r.examKind);
    var route = (r.stateCode && r.stateCode !== 'US') ? '/' + slug + '/' + r.stateCode.toLowerCase() : '/' + slug;
    return Object.assign({}, r, { route: route, title: r.shortName || r.examType });
  });
}
...
function loadTrackRegistry() {
  if (!trackRegistryPromise) {
    // The current page's content is gated alongside the registry fetch -- both are needed before
    // the first render, and they go out in parallel (the content file is preloaded by _worker.js,
    // so it usually costs no round trip at all).
    trackRegistryPromise = Promise.all([apiFetch('/track-registry'), loadPageTrackContent()]).then(function (results) {
      lastRegistryTracks = (results[0] && results[0].tracks) || [];
      HUB_EXAMS = buildHubExams(lastRegistryTracks);
    });
  }
...
// Hand-written hero copy for the categories that earn it, keyed by kind slug; every other category
// gets the generic per-kind template in renderCategoryPage(). CDL is here because it's the paid-ads
// destination: the headline matches how people search for it rather than echoing the registry's
// full "Commercial Driver (CDL)" label, and the subhead says what the product is instead of
// describing the exam. Requested 2026-09-17.
//
// Deliberately code, not the category-content CMS: _worker.js server-renders this same copy into
// the HTML (mirrored inside its SSR-HERO block, with test/landing-first-paint.test.js diffing the
...
function renderCategoryPage(kind) {
  ...
  var content = null;
  var override = CATEGORY_HERO_COPY[slug];
  var headline = override ? override.headline : fullKindLabel(kind) + ' Exam Prep';
  var subhead = override ? override.subhead
    : 'Practice questions for your state\'s ' + sentenceKindLabel(kind) + ' exam, built from official handbooks. Instant access, no subscription.';
...
function renderTrackLanding() {
  ...
  // Promoted into a badge in the hero's top row (see .exam-track-top below) rather than a small
  // muted footnote under the specs card, so this trust signal is visible without scrolling --
  // previously easy to miss entirely.
  var freshnessBadgeHtml = exam.questionsUpdatedAt
    ? '<span class="badge freshness-badge" title="Every question is sourced from the current official handbook or statute, not third-party prep material">🔄 Updated ' +
      new Date(exam.questionsUpdatedAt * 1000).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) +
      '</span>'
    : '';
  ...
  var intlStudentsHtml = intlSlug
    ...
    : '';
  // Study Resources row deliberately dropped -- trackResourceStatsHtml() right below this card
  // already shows the same counts as bold stat tiles, so a plain-text repeat here was redundant.
  ...
  appEl.innerHTML =
    ...
    // Same "resources need no login" reasoning as categoryHeroTrackLinkHtml's matching button --
    // a real, working preview the moment this page loads, not a locked teaser. Added alongside
    // the buy CTAs (prominent, above the fold) rather than only inside the "Preview the study
    // hub" widget further down, which visitors were missing entirely.
...
// ---- The deep bundle (js/app-deep.js) --------------------------------------------------------
// The static, legal and marketing routes -- #/terms, #/privacy, #/about, #/faq, #/guarantee,
// #/pass-rates, #/changelog, #/contact, the embed generator and /blog -- live in a second file that
// is fetched when one of them is visited, or at idle once a landing page is on screen. A visitor who
// reads a landing page and leaves never downloads it. See app-deep.js's own header for why this set.
//
// DEEP_BUNDLE_URL carries a content hash written by scripts/build-minified-js.js, so a changed deep
// bundle busts its own cache with no ?v= to keep in step by hand.
var DEEP_BUNDLE_URL = '/js/app-deep.min.js?v=DEEP_BUNDLE_HASH';
var DEEP_EXPORTS = {};
var deepBundlePromise = null;

// Called by app-deep.js as its last statement. Nothing else in this file may reference a function
// defined over there -- go through DEEP_EXPORTS, or it is a ReferenceError before the load.
function registerDeep(map) {
  Object.keys(map).forEach(function (key) { DEEP_EXPORTS[key] = map[key]; });
}

function deepBundleReady() { return !!DEEP_EXPORTS.renderTerms; }

function loadDeepBundle() {
  if (deepBundleReady()) return Promise.resolve();
  if (!deepBundlePromise) {
    deepBundlePromise = new Promise(function (resolve, reject) {
      var el = document.createElement('script');
      el.src = DEEP_BUNDLE_URL;
      el.async = true;
      el.onload = function () { resolve(); };
      // Cleared on failure, so the next navigation retries instead of being stuck on one flaky
      // request's rejected promise for the rest of the session.
      el.onerror = function () { deepBundlePromise = null; reject(new Error('app-deep.js failed to load')); };
      (document.head || document.documentElement).appendChild(el);
    });
  }
  return deepBundlePromise;
}

// hashView -> the DEEP_EXPORTS key that renders it.
var DEEP_HASH_VIEWS = {
  terms: 'renderTerms',
  ...
  'pass-rates': 'renderPassRates',
  embed: 'renderEmbedGenerator',
  changelog: 'renderChangelog',
};

// Renders a deep route, fetching the bundle first if this is the page load's first one. The
// skeleton is what the visitor sees for the length of that fetch -- never a blank page, and never
// the previous route's content left on screen.
function renderDeepRoute(exportName, arg) {
  if (deepBundleReady()) { DEEP_EXPORTS[exportName](arg); return; }
  appEl.innerHTML = loadingSkeletonHtml(6);
  loadDeepBundle().then(function () {
    DEEP_EXPORTS[exportName](arg);
  }).catch(function () {
    appEl.innerHTML = '<div class="card"><h1>Could not load this page</h1>' +
      '<p class="muted">Something went wrong fetching it \u2014 reloading the page usually fixes it.</p></div>';
  });
}
...
function route() {
  ...
  // The static/legal/marketing routes: rendered out of the deep bundle (see DEEP_HASH_VIEWS).
  if (DEEP_HASH_VIEWS[hashView]) { renderDeepRoute(DEEP_HASH_VIEWS[hashView]); return; }
  ...
  if (location.pathname === '/blog' || location.pathname === '/blog/') { renderDeepRoute('renderBlogList'); return; }
  var blogPostMatch = location.pathname.match(/^\/blog\/([a-z0-9-]+)\/?$/);
  if (blogPostMatch) { renderDeepRoute('renderBlogPost', blogPostMatch[1]); return; }
...
document.addEventListener('change', function (e) {
  if (e.target && e.target.name === 'claimType') {
    ...
    // DOM in place.
    var blogFilterHref = DEEP_EXPORTS.blogListHref &&
      DEEP_EXPORTS.blogListHref(e.target.getAttribute('data-kind') || '', e.target.value);
    if (blogFilterHref) location.href = blogFilterHref;
    ...
    if (embedPickerWrap && DEEP_EXPORTS.embedPickerHtml) embedPickerWrap.innerHTML = DEEP_EXPORTS.embedPickerHtml();
    var embedResultWrapKind = document.getElementById('embed-result-wrap');
    if (embedResultWrapKind && DEEP_EXPORTS.embedResultHtml) embedResultWrapKind.innerHTML = DEEP_EXPORTS.embedResultHtml();
    ...
    if (embedResultWrapState && DEEP_EXPORTS.embedResultHtml) embedResultWrapState.innerHTML = DEEP_EXPORTS.embedResultHtml();
  }
});
...
document.addEventListener('click', async function (e) {
  ...
  if (act === 'toggle-help-chat') {
    ...
    if (DEEP_EXPORTS.drawBlogList) DEEP_EXPORTS.drawBlogList();
...
(function boot() {
  ...
  Promise.all([loadSiteConfig(), loadAccountExamType(), loadTrackRegistry()]).then(function () {
    ...
    renderSiteFooter();
    // The other kinds' content, once this page is on screen: an in-app navigation to another
    // category, and the footer's cross-category links, want those real titles, and by now the
    // fetches cost the visitor nothing. Deliberately after route(), never in the gate above.
    if (window.requestIdleCallback) window.requestIdleCallback(loadRemainingTrackContent, { timeout: 4000 });
    else setTimeout(loadRemainingTrackContent, 2000);
    // Same bargain for the deep bundle: by the time someone taps through to the guarantee, the FAQ
    // or the blog it is already cached, while a two-second bounce never fetches it because idle
    // never comes. Failures are swallowed here -- renderDeepRoute retries on a real navigation.
    var prefetchDeep = function () { loadDeepBundle().catch(function () {}); };
    if (window.requestIdleCallback) window.requestIdleCallback(prefetchDeep, { timeout: 6000 });
    else setTimeout(prefetchDeep, 3000);
  });
})();
